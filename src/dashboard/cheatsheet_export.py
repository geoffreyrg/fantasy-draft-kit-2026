...
1. Numbered List ('1. Player Name') - Best for 'Import from Text'
2. Raw Name List ('Player Name')
3. WAF-Safe CSV ('Rank,Player,Team,Position,Tier,Notes') - Sanitized ASCII, no emojis/pipes
4. Minimal Clean CSV ('Rank,Player,Team,Position,Tier') - 100% upload compatibility
5. Positional Breakdowns (QB, RB, WR, TE tiers)
"""
...
def build_rich_tactical_note(row: pd.Series, ascii_safe: bool = True) -> str:
    """
    Constructs a rich, highly actionable, non-vague scouting note for cheat sheets.
    If ascii_safe is True, replaces emojis, pipes, and quotes with clean ASCII characters
    to prevent Cloudflare/WAF 403 Forbidden rejections during web paste/upload.
    """
    parts = []
    ...
    if des and des not in ["nan", "—", ""]:
        ...
        if "Target" in des_clean:
            parts.append("[TARGET] " + des_clean if ascii_safe else "🎯 " + des_clean)
        elif "Must-Have" in des_clean or "Exodia" in des_clean:
            parts.append("[EXODIA] " + des_clean if ascii_safe else "💥 " + des_clean)
        elif "Fade" in des_clean or "Overvalue" in des_clean or "Avoid" in des_clean:
            parts.append("[FADE] " + des_clean if ascii_safe else "🚫 " + des_clean)
        else:
            parts.append(des_clean)
    elif badge and badge not in ["nan", "—", ""]:
        if ascii_safe:
            b_clean = badge.replace("👑", "").replace("⚠️", "").replace("🏆", "").replace("🚀", "").strip()
            parts.append(f"[{b_clean}]")
        else:
            parts.append(badge)
    ...
    if row.get("is_contract_year") in [1.0, 1, "1", "1.0"]:
        parts.append("[Contract Year]" if ascii_safe else "💰 Contract Year")
    ...
    if pd.notnull(ol_rk) and str(ol_rk) != "nan":
        try:
            ...
            if ol_int <= 5:
                parts.append(f"[OL #{ol_int} Top-5]" if ascii_safe else f"🛡️ OL #{ol_int} (Top 5)")
            elif ol_int >= 28:
                parts.append(f"[OL #{ol_int} Poor]" if ascii_safe else f"⚠️ OL #{ol_int} (Poor)")
        except Exception:
            pass
    ...
    if "⭐⭐⭐⭐⭐" in p_sos or "Elite" in p_sos:
        parts.append("[Playoff A+]" if ascii_safe else "🏆 Playoff ⭐⭐⭐⭐⭐")
    ...
    if pd.notnull(adp_delta) and 4.0 <= float(adp_delta) <= 60.0 and adp_val <= 240.0:
        parts.append(f"[+{float(adp_delta):.1f} Value]" if ascii_safe else f"🟢 +{float(adp_delta):.1f} Value")
    ...
    # Format separator and clean quotes
    sep = " ; " if ascii_safe else " | "
    full_note = sep.join(parts)
    if ascii_safe:
        full_note = re.sub(r'[^\x00-\x7F]+', ' ', full_note)
        full_note = full_note.replace('"', '').replace("'", "").replace("|", ";")
        full_note = re.sub(r'\s+', ' ', full_note).strip(' ;')
    else:
        full_note = full_note.replace('"', "'")
    return full_note
...
def generate_fantasypros_exports(df: pd.DataFrame, top_n: int = 200) -> Dict[str, str]:
    ...
    # 3. WAF-Safe FantasyPros Standard CSV with Deep Tactical Scouting Notes
    csv_rows = ["Rank,Player,Team,Position,Tier,Notes"]
    simple_csv_rows = ["Rank,Player,Team,Position,Tier"]
    ...
    for rank, (_, row) in enumerate(combined_df.iterrows(), start=1):
        ...
        note = build_rich_tactical_note(row, ascii_safe=True)
        csv_rows.append(f'{rank},{name},{team},{pos},{tier},"{note}"')
        simple_csv_rows.append(f'{rank},{name},{team},{pos},{tier}')
    
    csv_text = "\n".join(csv_rows)
    simple_csv_text = "\n".join(simple_csv_rows)
    ...
    for p in ["RB", "WR", "TE", "QB", "K", "DST"]:
        ...
        for rk, (_, row) in enumerate(pos_df.iterrows(), start=1):
            ...
            p_note = build_rich_tactical_note(row, ascii_safe=True)
    ...
    (export_dir / "fantasypros_simple_rankings.csv").write_text(simple_csv_text, encoding="utf-8")
